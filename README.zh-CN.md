...
这八行的共同点只有一个：所有的分量都只压在对话里。

船也是一样，载重越轻越容易翻。所以船员会把重物压在舱底、压在一切之下。那份重量就叫 ballast（压舱物）。

**这是一个给 Claude 工作装上压舱物的插件。把规则、决定、验证过的事实压进文件里，即使是你毫无经验的领域，也能从那个目标真正需要的地基开始，一层层搭到完成。**

而且压舱物会累积。摸索半天才找到的方法、通过验证的事实、把庞大模糊的问题拆细后的结果 — 都会原样留下，下一个目标从它们之上出发。同一条路不挖第二遍。

上面第一行实际是怎么被拦住的，给你看一段。假设几周前埋过一条规则：
...
到这里是唯一由代码强制的部件做的事。算上其余十二个，部件是这样分的。
...
| **researcher** | 约定 — markdown skill | 把收集委托给接入的第二个 CLI — 采信不委托，结果以 `hearsay` 到达、必须过关 |
...
十三个部件不是各干各的，而是按顺序扣在同一个目标上。完整一圈是这样。
