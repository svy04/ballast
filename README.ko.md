...
- **설치하면 눈에 보이는 게 하나 생겨요** — 세션 시작의 한 줄(`[ballast] hook live — 아직 카탈로그 없음`, 규칙이 들어올 때까지). 나머지는 신호가 와야 깨어나요: 교정하면 pin이, 세션 첫 답변이면 recall이 깨어나요(`/ballast:brain-init` 전에는 recall이 훑을 기억 파일 자체가 없어요).
