...
**ballast는 해본 적 없는 분야의 목표라도 거기 필요한 기반부터 쌓아 올려 끝까지 데려가는 플러그인이에요. 목표를 필요한 기반으로 쪼개고, 이미 가진 것은 먼저 꺼내 쓰고, 없는 칸은 배워서 채우고, 한 번 뚫은 길은 규칙·검증된 기록·절차로 남아 다음 목표가 거기서 시작해요. 완료는 검사를 통과해야 완료예요.**
...
<p align="center">
  <a href="CHANGELOG.md"><img src="https://img.shields.io/badge/version-0.6.0-blue" alt="버전 0.6.0"></a>
...
| 문제 | 증상 | 처방 |
|---|---|---|
| `CLAUDE.md`는 한 번 읽히고 끝 — 일이 진행될수록 멀어져요 | 같은 교정을 다음번에 또 | **rules hook** *(코드)* — 걸린 규칙 본문을 메시지마다 배달 |
...
- **준비** — goal이 프로젝트가 이미 가진 것부터 꺼내 쓰고, 그다음 지형을 살펴요 — 리서처를 연결해 두면 수집 자체를 맡길 수도 있어요
...
checkpoint(복귀 지점)는 그 사슬을 쉬는 동안 이어 줘요 — 목표로 돌아오는 일이 30초 읽기가 돼요.
