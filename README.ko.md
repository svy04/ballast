...
`[ballast]` 블록이 ballast가 보장하는 부분이에요. "npm"이 규칙에 걸려 규칙 본문 전체가 이 메시지와 함께 도착했고, 아래 답은 Claude가 기억해서가 아니라 건네받은 규칙을 따른 결과예요.
...
- **설치만으로는 아무것도 달라지지 않아요** — 훅은 빈 채로 출발하고, 스킬은 신호가 와야 깨어나는 규약이에요: 교정하면 pin이, 세션 첫 답변이면 recall이 깨어나요(`/ballast:brain-init` 전에는 recall이 훑을 기억 파일 자체가 없어요).
  `BALLAST_DISABLE=1`은 훅을 끄고, 스킬은 플러그인을 지우면 같이 나가요.
