...
- **같은 교정을 다음 달에 또 한다** — 걸리는 메시지마다 그 규칙이 같이 와요
- **끝난 결정이 다시 열리거나 조용히 바뀐다** — 결정은 고치지 않고 새로 덧붙여요
- **출처 없는 답을 믿었다가 나중에 안다** — 먼저 반박을 받고 견뎌야 사실이 돼요
- **아직 없는 기능을 카피가 있다고 쓴다** — 밖에 하는 말은 증거 있는 것만 써요
- **"다 됐다"의 근거가 Claude 말뿐이다** — 검사를 통과해야 완료라고 해요
- **내보낸 결과물이 첫 독자에게서 막힌다** — 아무것도 모르는 사람이 먼저 해봐요
- **다른 AI가 정리해 준 게 사실로 앉는다** — 찾아오는 건 맡겨도 믿을지는 안 맡겨요
- **이미 적어둔 걸 못 찾고 또 조사한다** — 답하기 전에 가진 것부터 전부 훑어요

여덟 줄의 공통점은 하나예요. 무게가 전부 대화 안에만 실려 있다는 것.

배도 짐이 가벼울수록 잘 뒤집혀요. 그래서 밑바닥에 일부러 무게를 싣습니다. 그 무게를 ballast라고 해요.

**이건 Claude와 하는 일에 그 밑짐을 싣는 플러그인이에요. 규칙과 결정과 검증된 사실을 파일에 실어두면, 해본 적 없는 분야의 목표라도 거기 필요한 기반부터 쌓아 올려 끝까지 갈 수 있어요.**

그리고 밑짐은 쌓여요. 헤매다 겨우 찾은 방법, 확인을 통과한 사실, 막연하고 큰 문제를 잘게 쪼갠 결과 — 그게 그대로 남아서 다음 목표는 그 위에서 시작해요. 같은 길을 두 번 파지 않아요.

위 첫 줄이 실제로 어떻게 막히는지 보여드릴게요. 몇 주 전에 규칙을 한 번 심어뒀다면:
...
- **의존성 0, 네트워크 0** — 훅은 스크립트 하나고, 임포트는 `fs`/`os`/`path`뿐이에요. ballast 자체는 아무것도 밖으로 안 보내요(선택으로 연결하는 verifier/researcher 명령은 사용자가 고른 로컬 CLI고, 검사 하니스는 별개 스크립트라 `node`를 띄워 훅만 다시 돌려요)
...
첫 화면의 여덟 줄이 왜 생기는지, 어느 조각이 그걸 맡는지예요. *코드*는 스크립트가 강제한다는 뜻이고, *규약*은 Claude가 따르기로 한 마크다운 지침이라는 뜻이에요.

| 왜 그런가 | 맡는 조각 |
|---|---|
| `CLAUDE.md`는 한 번 읽히고 끝 — 일이 진행될수록 멀어져요 | **rules hook** *(코드)* — 걸린 규칙 본문을 메시지마다 배달 |
| 결정이 지난 대화 속에 묻혀요 | **decision ledger**(결정 원장) *(규약)* — 덧붙이기만, 번복은 대체(supersede) 기록 |
| 그럴듯한 문장이 사실로 굳어요 | **verify gate**(검증 관문) *(규약)* — 주장마다 라벨, `confirmed`(확인됨)는 얻어야 함 |
| 카피가 제품이 아니라 로드맵을 설명해요 | **proof standard**(증거 기준) *(규약)* — 대외 주장은 진실 파일에서만 |
| "완료"의 근거가 Claude의 말뿐이에요 | **goal** *(규약)* — 완료 = 검사 통과 |
| 결과물이 독자를 처음 만나는 순간 무너져요 | **rehearsal**(리허설) *(규약)* — 맥락 없는 독자가 내보내기 전에 실제로 실행해요 |
| 수집과 판정이 한 몸으로 움직여요 | **researcher**(리서처) *(규약)* — 수집은 맡겨도 판정은 안 맡겨요, 결과는 `hearsay`로 도착 |
| 적어둔 것이 색인까지 돼 있는데 안 열려요 | **recall**(전수 조회) *(규약)* — 세션 첫 답변과 주제가 바뀔 때, 답하기 전에 다섯 곳을 전부 훑어요 |

진실 파일(`memory/PRODUCT-TRUTH.md`)은 제품이 실제로 하는 일을 증거·날짜와 함께 적어둔 기록이에요. 검사 통과는 파일이 있고, 테스트가 돌고, 산출물을 실제로 봤다는 뜻이에요.

여덟 줄 중 코드로 강제되는 건 한 줄이에요. rules hook은 프롬프트마다 실행되는 스크립트라, Claude가 협조하든 말든 발동해요.

나머지 일곱은 규약이에요 — Claude가 따라 주는 만큼만 지켜지고, 여느 프롬프트처럼 흐려질 수 있어요. ballast는 이걸 아닌 척하지 않아요.
...
- **준비** — recall이 세션 첫 답변과 주제 전환마다 가진 것을 전부 훑고, goal이 그걸 과제로 끌어와 쓴 뒤 지형을 살펴요 — 리서처를 연결해 두면 수집 자체를 맡길 수도 있어요
- **축적** — 일하면서 검증된 것을 knowledge-base(지식 창고)와 결정 원장에 쌓아요
- **재사용·복제** — rules hook · pin · skill-forge(스킬 대장간)가 쌓인 것을 다음 세션에, 그리고 다음 목표에 되돌려요
- **품질 보장** — verify-gate가 주장과 `confirmed` 사이에 서고, 리허설이 결과물과 독자 사이에 서고, goal은 검사를 통과하기 전엔 완료라고 안 해요

checkpoint(복귀 지점)는 그 사슬을 쉬는 동안 이어 줘요 — 목표로 돌아오는 일이 30초 읽기가 돼요. 같은 사슬을 목표 하나가 지나가는 경로로 그리면 이래요. 네모는 조각이고, 원통은 세션이 끝나도 남는 파일이에요.
...
여기까지가 코드로 강제되는 조각 하나가 하는 일이에요. 나머지 열두 개까지, 구성 조각은 이렇게 나뉘어요.
...
| **goal** | 규약 — 마크다운 스킬 | 가진 것부터 동원하고, 목표를 위에서부터 겹침도 빠짐도 없이 원자 단위 피라미드로 쪼갠 뒤, 빈칸을 아래에서부터 검증해 가며 채워요 |
...
열세 조각이 따로 노는 게 아니라 목표 하나에 순서대로 붙어요. 그 한 바퀴가 이래요.

## 목표 하나, 처음부터 끝까지

1. `/ballast:goal 가격 페이지 만들기` — 동원이 카탈로그의 가격 규칙과 `memory/knowledge/`의 브랜드 사실부터 찾아요. 둘 다 다시 조사하지 않고 그대로 써요.
2. 한 가지(결제 카피 관례)는 빈칸이에요. 지형을 훑고, 목표를 위에서부터 원자 단위로 쪼개고, 빈칸을 아래에서부터 채워요 — 리서처가 있으면 수집은 맡기고(전부 `hearsay`로 돌아와요), 관문을 통과한 것만 `memory/knowledge/`에 쌓여요.
...
단 이 여섯 단계는 카탈로그에 규칙이 한 줄이라도 들어간 뒤부터 돌아요. 그 한 줄을 심는 게 빠른 시작이에요.
...
- **조용한 실패, 예외 하나** — 정규식이 틀려도 내부 오류가 나도 세션은 안 깨지고 아무 말도 안 해요. 예외는 카탈로그가 있는데 안 읽히는 경우예요 — 이때만 한 줄로 알려줘요. 안 그러면 규칙이 통째로 빠진 상태와 그냥 안 걸린 상태가 똑같아 보이거든요.
- **fail-open** — 훅이 아예 못 도는 상황(PATH에 `node` 없음, 카탈로그 읽기 실패)에서는 `block` 규칙도 같이 안 걸려요. 차단은 가드레일로 쓰시고, 뚫리지 않는 샌드박스로 믿지는 마세요.

fail-open엔 오류 화면이 없어요 — 걸렸어야 할 메시지에 `[ballast]` 블록이 안 뜨는 게 유일한 증상이에요. 그 증상이 보이면 셋을 순서대로 확인하세요.

1. `node --version`이 18 이상인지 확인하세요 — 없으면 Node부터 설치하고요
2. 방금 설치했다면 Claude Code를 재시작하세요
3. `BALLAST_DEBUG=1`로 다시 돌려 원인을 보세요

훅이 살아난 걸 확인했으면 첫 푸시 전에 하나 더 있어요. Claude가 굴려온 저장소는 [docs/PUBLISH-CHECKLIST.md](docs/PUBLISH-CHECKLIST.md)(영문)를 한 줄씩 밟으세요 — 이런 작업 공간에는 어느새 안 들여다보게 된 파일들에 비밀값이 쌓여요.
...
대신 장치는 확인할 수 있어요 — 훅, 스킬 열두 개, 규칙 형식이 전부 이 저장소에 있고, 한나절이면 다 읽혀요. 루프의 선례를 아시면 [이슈](https://github.com/svy04/ballast/issues)로 알려주세요. 링크해 둘게요.
