...
- **Example catalog cleaned up.** The dead `always-on-example` entry is gone (its advice moved into the file's comment); the danger-block example says what it is — it stops the prompt, not the tool; the four inject rules carry Chinese keywords beside English and Korean.
- **Honesty edits, from an outside review and a landscape sweep run on 2026-08-21.** README: the harness checks what the hook emits, not whether the model obeys; the demo's reply line is marked illustrative; specs state the standing cost (the twelve skill descriptions are about 490 words; skill bodies load only when a skill is invoked; the hook adds nothing until a rule matches); the limits section says a cloned repo's project catalog is someone else's standing instructions. CONTRIBUTING's mirror rule covers the Chinese README; rehearsal no longer cites a round log this repo does not contain; `docs/CODEX.md` says ballast does not touch providers or routing.
...
- **Reported by [@biggieb327-lgtm](https://github.com/biggieb327-lgtm) in [#2](https://github.com/svy04/ballast/pull/2)** — the first outside contribution to ballast. The commit in that PR carries Claude's authorship, so GitHub's contributor list credits the bot account rather than the person; the credit belongs here.
